    ...
    return (
        <div className="max-w-4xl mx-auto p-1 bg-gray-300 dark:bg-gray-800 rounded-lg shadow-lg">
            ...
            <div className="flex flex-row justify-center space-x-4">
                                ...
                                <div className="flex flex-row text-sm">
                                    <div>
                                        <div className="flex flex-row mt-1 mr-2">
                                            <p className="text-gray-600 dark:text-gray-300 mr-2">Preço Unitário: ${item.product.price.toFixed(2)}</p>
                                            <p className="text-gray-600 dark:text-gray-300 ml-2">Preço Total/Item: <strong>${(item.product.price * item.quantity).toFixed(2)}</strong></p>
                                        </div>
                                        <div className="flex flex-row items-center">
                                            ...
                                            <input
                                                ...
                                                className="m-2 border rounded p-1 w-1/4"
                                            />
                                            <div className="relative">
                                                ...
                                                {tooltipState[item.product.name] && (
                                                    <div className="absolute bg-black text-white text-xs px-2 py-1 rounded mt-2 -ml-8">
                ...
                <div>
                    {cart.items.slice(4).map((item, index) => (
                        <div key={index} className="flex items-center p-4 m-1 bg-white dark:bg-gray-700 rounded-lg shadow">
                            <div className="w-20 h-20 mr-4">
                                <Image src={item.product.image_url} alt={item.product.name} width={80} height={80} className="rounded-lg object-cover" priority={true} />
                            </div>
                ...
                <div className="mt-1 mr-3 grow flex">
                    ...
                    >
                        ...
                        {tooltipState.restoreCart && (
                            <div className="absolute bg-black text-white text-xs px-2 py-1 rounded mt-16 ml-2">
                                Restaurar carrinho
                            </div>
                        )}
                    </button>
                </div>
                <p className="text-2xl font-bold text-gray-900 dark:text-gray-100">Total: R${cart.totalPrice.toFixed(2)}</p>
            </div>
        </div>
    )
}
